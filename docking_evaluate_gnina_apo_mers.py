...
POCKET_DIR = "./data/MERS-pocket-dir"
# for apo_id in tqdm(all_apo_df['apo_id']):   
all_apo_ids = os.listdir(POCKET_DIR)
for apo_id in tqdm(all_apo_ids):
    start_time = time.time()
    p_id = apo_id.split("_")[0]
    l_id = apo_id.split("_")[1]
    # search_pattern = os.path.join(POCKET_DIR, p_id, "Ligands", l_id + "*.pdb")
    # matching_files = glob.glob(search_pattern)
    SDF_file = f"{POCKET_DIR}/{apo_id}/ligand.pdb"
    mol2_file = SDF_file
    ori_LIG_pdb_file = SDF_file
    ori_protein_path = f"./data/full_run-MERS/mers-protein/protein.pdb"
    protein_path = f"{POCKET_DIR}/{apo_id}/protein_pocket.pdb"
